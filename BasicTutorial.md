...
- Then go to Windows->AI->Navigation and select "Bake". This will show you blue spots on the scene view, these are the areas which your AI can walk over, if they don't seem right you might be missing some static objects or you may have overlapping objects. You need to do these 2 steps only once for all environment agents. If you add more structures or paths then bake again
	- If you want to add "invisible" paths that only characters can walk on but players cannot (e.g. drones floating in the air) you can create planes in the air, remove their collider component and disable their mesh renderer. This will make it invisible and the player can no longer interact with them, but if you place waypoints on these surfaces characters that can reach them will move in between these waypoints. You will of course have to bake again
